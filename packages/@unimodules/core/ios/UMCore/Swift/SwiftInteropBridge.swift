...
public class SwiftInteropBridge: NSObject {
  ...
  @objc
  public init(modulesProvider: ModulesProvider) {
    self.registry = ModuleRegistry(withProvider: modulesProvider)
    super.init()
  }
  ...
  public func exportedModulesConstants() -> [String: Any] {
    return registry.reduce(into: [String: Any]()) { acc, holder in
      acc[holder.module.name] = holder.module.constantsToExport()
    }
  }
}
